...
---
> SQLite3 引入了一个十分显眼的机制write-ahead logging机制，该机制相对于原先的logging机制
可以更高程度的支持数据库并发操作，在数据库操作过程中涉及到更少量的IO操作。
...
**WAL模式的延续性表明了应用程序无需进行任何改变就可以将日志更改为WAL模式，只需要使用shell或则其他工具在运行PRAGMA journal mode=WAL
指令并重启程序。WAL日志模式将会被设置到同一个数据库上的所有数据库连接。**
...
"version 2"类型的共享内存。如果虚拟文件系统不支持共享内存，那么WAL模式下打开数据库或则将数据库日志模式切换成WAL模式都将失败。
